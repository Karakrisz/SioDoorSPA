...
useHead({
  titleTemplate: (titleChunk) => {
    return titleChunk ? `${titleChunk} -  Vasalás Mester` : ' Vasalás Mester';
  }
})
</script>
...
pre {
  @apply rounded bg-stone-800 my-8 text-xs text-white p-4 whitespace-pre-wrap overflow-auto;
}
...
.custom-scrollbar::-webkit-scrollbar {
  @apply rounded bg-stone-100 w-1.5;
}

.custom-scrollbar::-webkit-scrollbar-thumb {
  @apply rounded bg-stone-400;
}
